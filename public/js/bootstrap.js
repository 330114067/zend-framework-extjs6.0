...
Ext.Microloader = Ext.Microloader || (function() {
	var Boot = Ext.Boot, _warn = function(message) {
		console.log("[WARN] " + message);
	}, _privatePrefix = '_ext:' + location.pathname,

	/**
	 * @method getStorageKey The Following combination is used to create
	 *         isolated local storage keys '_ext' is used to scope all the local
	 *         storage keys that we internally by Ext 'location.pathname' is
	 *         used to force each assets to cache by an absolute URL
	 *         (/build/MyApp) (dev vs prod) 'url' is used to force each asset to
	 *         cache relative to the page (app.json vs resources/app.css)
	 *         'profileId' is used to differentiate the builds of an application
	 *         (neptune vs crisp) 'Microloader.appId' is unique to the
	 *         application and will differentiate apps on the same host (dev
	 *         mode running app watch against multiple apps)
	 */
	...
	var _listeners = [], _loaded = false, Microloader = {
		...
		applyCacheBuster : function(url) {
			var tstamp = new Date().getTime(), sep = url.indexOf('?') === -1
					? '?'
					: '&';
			...
			if (typeof manifest === "string") {
				...
				if (content) {
					...
					if (location.href.indexOf('file:/') === 0) {
						...
						Boot.fetch(Microloader.applyCacheBuster(url), function(
										result) {
