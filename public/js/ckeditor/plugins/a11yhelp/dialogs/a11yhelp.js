...
CKEDITOR.dialog.add("a11yHelp", function(h) {
	var a = h.lang.a11yhelp, b = h.lang.common.keyboard, n = CKEDITOR.tools
			.getNextId(), e = {
		8 : b[8],
		9 : a.tab,
		13 : b[13],
	...
	var g = [CKEDITOR.ALT, CKEDITOR.SHIFT, CKEDITOR.CTRL], p = /\$\{(.*?)\}/g, t = function() {
	...
	return {
		...
		contents : [{
			...
			elements : [{
				...
				html : function() {
					for (var b = '\x3cdiv class\x3d"cke_accessibility_legend" role\x3d"document" aria-labelledby\x3d"'
							+ n
							+ '_arialbl" tabIndex\x3d"-1"\x3e%1\x3c/div\x3e\x3cspan id\x3d"'
							+ n
							+ '_arialbl" class\x3d"cke_voice_label"\x3e'
							+ a.contents + " \x3c/span\x3e", e = [], d = a.legend, h = d.length, g = 0; g < h; g++) {
						for (var c = d[g], k = [], l = c.items, m = l.length, f = 0; f < m; f++) {
							var q = l[f], r = q.legend.replace(p, t);
							r.match(p)
									|| k
											.push("\x3cdt\x3e%1\x3c/dt\x3e\x3cdd\x3e%2\x3c/dd\x3e"
													.replace("%1", q.name)
													.replace("%2", r))
						}
						e.push("\x3ch1\x3e%1\x3c/h1\x3e\x3cdl\x3e%2\x3c/dl\x3e"
								.replace("%1", c.name)
								.replace("%2", k.join("")))
					}
