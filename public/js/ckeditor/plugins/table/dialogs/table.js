...
(function() {
	...
	function q(a, f) {
		...
		return {
			...
			onShow : function() {
				var e = a.getSelection(), d = e.getRanges(), b, c = this
						.getContentElement("info", "txtRows"), g = this
						.getContentElement("info", "txtCols"), t = this
						.getContentElement("info", "txtWidth"), m = this
				...
				"tableProperties" == f
						&& ((e = e.getSelectedElement()) && e.is("table")
								...
								: 0 < d.length
										&& (CKEDITOR.env.webkit
												&& d[0]
														.shrink(CKEDITOR.NODE_ELEMENT), b = a
												.elementPath(d[0]
														.getCommonAncestor(!0))
												.contains("table", 1)), this._.selectedElement = b);
				b
						? (this.setupContent(b), c && c.disable(), g
								&& g.disable())
			...
			onOk : function() {
				var e = a.getSelection(), d = this._.selectedElement
						&& e.createBookmarks(), b = this._.selectedElement
						|| n("table"), c = {};
				this.commitContent(c, b);
				if (c.info) {
					c = c.info;
					if (!this._.selectedElement)
						for (var g = b.append(n("tbody")), f = parseInt(
								c.txtRows, 10)
								|| 0, m = parseInt(c.txtCols, 10) || 0, k = 0; k < f; k++)
					...
					if (!this.hasColumnHeaders && ("col" == f || "both" == f))
						for (h = 0; h < b.$.rows.length; h++)
							g = new CKEDITOR.dom.element(b.$.rows[h].cells[0]), g
									.renameNode("th"), g.setAttribute("scope",
									"row");
					if (this.hasColumnHeaders && "col" != f && "both" != f)
						for (k = 0; k < b.$.rows.length; k++)
							h = new CKEDITOR.dom.element(b.$.rows[k]), "tbody" == h
									.getParent().getName()
									&& (g = new CKEDITOR.dom.element(h.$.cells[0]), g
											.renameNode("td"), g
											.removeAttribute("scope"));
					c.txtHeight ? b.setStyle("height", c.txtHeight) : b
							.removeStyle("height");
					c.txtWidth ? b.setStyle("width", c.txtWidth) : b
							.removeStyle("width");
				...
				if (this._.selectedElement)
					...
					a.insertElement(b), setTimeout(function() {
						var e = new CKEDITOR.dom.element(b.$.rows[0].cells[0]), c = a
								.createRange();
				...
				elements : [{
					...
					children : [{
						...
						setup : function(a) {
							...
							if (0 < a.count()) {
								a = a.getItem(0);
								var d = a.getFirst(CKEDITOR.dom.walker
								...
								if (b)
									0 < c.count()
											...
											: (c = new CKEDITOR.dom.element(
													...
													.append(c, !0)), c
											.append(new CKEDITOR.dom.text(b,
													a.document));
								else if (0 < c.count())
