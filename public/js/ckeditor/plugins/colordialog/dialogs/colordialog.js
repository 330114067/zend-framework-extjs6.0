...
CKEDITOR.dialog.add("colordialog", function(x) {
	...
	function z(a) {
		...
		"td" == a.getName()
				...
				&& (y(), f = a, f.setAttribute("aria-selected", !0), f
						.addClass("cke_colordialog_selected"), p
						.getContentElement("picker", "selectedColor")
						.setValue(c))
	}
	function y() {
		f
				&& (f.removeClass("cke_colordialog_selected"), f
						.removeAttribute("aria-selected"), f = null)
	}
	function D(a) {
		a = a.replace(/^#/, "");
		for (var c = 0, b = []; 2 >= c; c++)
		...
		"td" == b.getName()
				...
				&& (q(a), c ? d = b : B = b, c
						&& b.addClass(D(g)
								...
								: "cke_colordialog_focused_dark"), r(g))
	}
	function q(a) {
		if (a = !/mouse/.test(a.name) && d)
			a.removeClass("cke_colordialog_focused_light"), a
					.removeClass("cke_colordialog_focused_dark");
		d || B || r(!1)
	}
	function r(a) {
		a ? (h.getById(t).setStyle("background-color", a), h.getById(u)
				.setHtml(a)) : (h.getById(t).removeStyle("background-color"), h
				.getById(u).setHtml("\x26nbsp;"))
	}
	function E(a) {
		...
		switch (g) {
			...
			case 40 :
				(a = b.getParent().getNext())
						&& (a = a.getChild([b.getIndex()])) && 1 == a.type
						&& a.focus();
				c.preventDefault();
				break;
			case 32 :
			case 13 :
				...
				break;
			case e ? 37 : 39 :
				(a = b.getNext()) ? 1 == a.type
						&& (a.focus(), c.preventDefault(!0)) : (a = b
						.getParent().getNext())
						&& (a = a.getChild([0]))
						&& 1 == a.type
...
(function() {
		...
		function c(a, c) {
			...
			b.append(CKEDITOR.dom.element.createFromHtml('\x3cspan id\x3d"' + d
							+ '" class\x3d"cke_voice_label"\x3e' + c
							+ "\x3c/span\x3e", CKEDITOR.document))
		}
		k = CKEDITOR.dom.element
				.createFromHtml('\x3ctable tabIndex\x3d"-1" class\x3d"cke_colordialog_table" aria-label\x3d"'
						+ e.options
	...
	CKEDITOR.document.appendStyleSheet(CKEDITOR.getUrl(CKEDITOR.plugins
			.get("colordialog").path
		...
		contents : [{
			...
			elements : [{
				...
				children : [{
					...
					focus : function() {
						(d || this.getElement().getElementsByTag("td")
								.getItem(0)).focus()
					}
				}, C, {
					type : "vbox",
					...
					children : [{
						...
						onChange : function() {
							try {
								h.getById(n).setStyle("background-color",
										this.getValue())
							} catch (a) {
								m()
							}
